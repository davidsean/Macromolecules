...
# the cylinder radius (should not be below 1)

	set cyl_radius 3.0
	constraint pore center [expr 0.5*$lx] [expr 0.5*$ly] [expr 0.5*$lz] axis 1 0 0 radius $cyl_radius length [expr 0.5*$lx] type $type_wall
...
# Tubes can be tricky, it is better to start as a straight line an warm up!
# place the monomers in a line!

	for {set p 0} {$p<$n_mono} {incr p} {
	 part $p pos [expr 0.001+$p*$b] [expr 0.5*$ly] [expr 0.5*$lz] type $type_mono 
	}

# define FENE bonds between neighbors
	for {set p 1} {$p<$n_mono} {incr p} {
	 part $p bond $bondID_fene [expr $p-1]
	}
...
# extra warm-up, because we have a "cold"-start
	thermostat langevin $kBT 0.01
	integrate 10000
	thermostat langevin $kBT $gamma
