...
#############################################################
#see PRL-A, Volume 33, number 5, May 1986, Gary S. Grest and Kurt Kremer
	set bondID_angle $inter_ID
	incr inter_ID

# stiffness k, traditional value is 7
	set k_angle 10.0
# delta r_max, traditional value is 1.5
	set theta_angle [expr 3.141592653]  
			
	inter $bondID_angle angle $k_angle $theta_angle
...
# create using a Pruned Self-Avoiding Random Walk (WARNING: NOT A RANDOM WALK)
	polymer 1 $n_mono $b start 0 pos [expr 0.001] [expr 0.5*$ly] [expr 0.5*$lz]  mode PSAW [expr .99*$b] $try_max types $type_mono bond $bondID_fene constraints

# this activates a bending potential
